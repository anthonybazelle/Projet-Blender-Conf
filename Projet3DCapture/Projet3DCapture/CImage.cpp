...
int CImage::main(int argc, char **argv)
{
...
float CImage::cv_distance(Point2f P, Point2f Q)
{
...
float CImage::cv_lineEquation(Point2f L, Point2f M, Point2f J)
{
	float a,b,c,pdist;
...
float CImage::cv_lineSlope(Point2f L, Point2f M, int& alignement)
{
	float dx,dy;
	dx = M.x - L.x;
	dy = M.y - L.y;
...
void CImage::cv_getVertices(vector<vector<Point> > contours, int c_id, float slope, vector<Point2f>& quad)
{
	Rect box;
...
void CImage::cv_updateCorner(Point2f P, Point2f ref , float& baseline,  Point2f& corner)
{
    float temp_dist;
...
void CImage::cv_updateCornerOr(int orientation, vector<Point2f> IN,vector<Point2f> &OUT)
{
	Point2f M0,M1,M2,M3;
    	if(orientation == CV_QR_NORTH)
	{
...
bool CImage::getIntersectionPoint(Point2f a1, Point2f a2, Point2f b1, Point2f b2, Point2f& intersection)
{
    Point2f p = a1;
    Point2f q = b1;
    Point2f r(a2-a1);
...
float CImage::cross(Point2f v1,Point2f v2)
{
